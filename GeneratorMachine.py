...
class VAE(tf.keras.Model):
    ...
    def call(self, x):
        ...
        # Use decoder output as the reconstruction mean
        recon_logp = log_diag_mvn(x, self.decoder.mu, tf.math.log(self.decoder.std))
        # Use encoder outputs to compute KL divergence
        kl = kl_divergence(self.encoder.mu, self.encoder.log_var)
        self.vae_loss = - (recon_logp - kl)
    ...
    def plot_grid(images,N,C,name, figsize=(24., 28.)):
        '''
    ...
    def visualize_latent_space(self,test_X, num_samples=1000, perplexity=30, learning_rate=200):
        '''
        Visualize the latent space using t-SNE.
        '''
        # Randomly sample from the dataset
        idx = np.random.choice(test_X.shape[0], num_samples, replace=False)
        x_samples = tf.gather(test_X, idx)

        # Encode to latent space
        z_samples = self.encoder(x_samples).numpy()  # shape (num_samples, latent_dim)

        # Apply t-SNE to reduce to 2D
        tsne = TSNE(n_components=2, perplexity=perplexity, learning_rate=learning_rate, random_state=42)
        z_embedded = tsne.fit_transform(z_samples)

        # Plot with matplotlib
        plt.figure(figsize=(10, 8))
        plt.scatter(z_embedded[:, 0], z_embedded[:, 1], c='blue', alpha=0.6, s=10)
    ...
    def generate_from_posterior(self, test_X, num_samples=100, n=10, C=10, name='posterior'):
        '''
        ...
        self.plot_grid(x_decoded, n, C, name)
    
    def generate_from_prior(self, num_samples=100, n=10, C=10, name='prior'):
        '''
